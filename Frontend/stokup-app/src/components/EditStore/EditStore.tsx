...
import { LoaderCircle, Store as StoreIcon } from 'lucide-react';

type EditStoreProps = {
    store: Store; 
    onCancel: () => void;
    onSuccess: () => Promise<void>;
    onEdit: (id: string, payload: Partial<Store>) => Promise<void>;
    error: boolean;
    edited: boolean;
    loading: boolean; 
};
...
export function EditStore({
    store, onCancel, onSuccess, onEdit, error, loading, edited}: EditStoreProps){
    ...
    return (
        <div className="flex flex-col p-6">
            <h3 className="text-[1.8rem] font-semibold text-[#3B82F6]">Editar Loja</h3>

             {error && (
                <div className='mt-6'>
                <FormsError text={"Erro ao editar loja, tente novamente mais tarde."}/>
                </div>
            )}

             {edited && (
                <div className='mt-6'>
                <FormSuccess text={"Loja alterada com sucesso."}/>
                </div>
            )}
            ...
            text-sm rounded-lg border-2 border-blue-300 bg-blue-50 text-blue-600 font-semibold'>
                ...
                <div className='px-2 pl-4 flex items-center'>

                    <p>Código: {store.store_id}</p>
